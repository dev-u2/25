...
async function Solving(conn, store) {
    ...
    conn.getName = async (jid, withoutContact = false) => {
        ...
        if (id.endsWith('@g.us')) {
            const groupMetadata = store.contacts[id] || await conn.groupMetadata(id) || {};
    ...
    conn.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
        ...
        if (!mimeType || mimeType.includes('octet-stream')) {
            const detectedType = await fileType.fromBuffer(response.data);
    ...
    conn.sendAsSticker = async (jid, buffer, quoted, options = {}) => {
        const imageBuffer = Buffer.isBuffer(buffer) ? buffer : /^data:.*?\/.*?;base64,/i.test(buffer) ? Buffer.from(buffer.split`,`[1], 'base64') : /^https?:\/\//.test(buffer) ? await await getBuffer(buffer) : fs.existsSync(buffer) ? fs.readFileSync(buffer) : Buffer.alloc(0);
    ...
    conn.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
        ...
        const type = await fileType.fromBuffer(downloaded);
    ...
    conn.getFile = async (path, saveToFile) => {
        ...
        filename = path.join(__dirname, './lib/database/sampah/' + new Date() * 1 + '.' + type.ext);
    ...
    if (conn.user && conn.user.id) {
        ...
        if (global.db?.data[botJid]) {
            conn.public = global.db.data[botJid].public;
        } else {
            conn.public = true;
        }
    } else {
...
async function Serialize(conn, message, store, groupMetadata) {
    ...
    return message.message && (
        ...
        message.prefix = config.PREFIX.test(message.body) ? message.body.match(config.PREFIX)[0] : /[\uD800-\uDBFF][\uDC00-\uDFFF]/gi.test(message.body) ? message.body.match(/[\uD800-\uDBFF][\uDC00-\uDFFF]/gi)[0] : '',
        ...
        message.quoted && (
            ...
            message.quoted.body = message.quoted.msg?.text || message.quoted.msg?.caption || message.quoted?.message?.conversation || message.quoted.msg?.selectedButtonId || message.quoted.msg?.singleSelectReply?.selectedRowId || message.quoted.msg?.selectedId || message.quoted.msg?.contentText || message.quoted.msg?.selectedDisplayText || message.quoted.msg?.title || message.quoted?.msg?.name || '',
            message.quoted.prefix = config.PREFIX.test(message.quoted.body) ? message.quoted.body.match(config.PREFIX)[0] : /[\uD800-\uDBFF][\uDC00-\uDFFF]/gi.test(message.quoted.body) ? message.quoted.body.match(/[\uD800-\uDBFF][\uDC00-\uDFFF]/gi)[0] : '',
                    ...
                    if (/^https?:\/\//.test(content)) {
                        ...
                        if (!isGroup && config.tagAI) {
                            return conn.sendMessage(chat, {
                                ...
                                quoted: quoted,
                                ephemeralExpirationn: ephemeralExpirationn,
                                additionalNodes: [{ tag: 'bot', attrs: { biz_bot: '1' } }]
                            });
                        }
                        return conn.sendMessage(chat, {
