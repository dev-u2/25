    ...
    DataBase = class MongoDB {
        constructor(url = config.database, options = { useNewUrlParser: true, useUnifiedTopology: true, serverSelectionTimeoutMS: 5000 }) {
            ...
            mongoose.connection.on("disconnected", async () => {
                ...
                console.warn("❗ Perte de connexion MongoDB, reconnexion dans 5 secondes..");
        ...
        connect = async(retries = 5, delay = 2000) => {
            ...
            while (retries > 0) {
                try {
                    console.log(`🔄 Tentative de connexion à MongoDB... (Tentative de connexion ... ${6 - retries}/5)`);
            ...
            this.isConnecting = false;
            throw new Error("❌ Échec de la connexion à MongoDB après plusieurs tentatives.");
        };
        read = async () => {
            if (mongoose.connection.readyState !== 1 && !this.isConnecting) {
                await this.connect();
