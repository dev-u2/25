...
async function startRavBot() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if ((connection == 'connecting' || !!qr) && pairingCode && !sock.authState.creds.registered && !pairingStarted) {
      pairingStarted = true;
      let phoneNumber;
      ...
      setTimeout(async () => {
        await getPhoneNumber();
        await exec('rm -rf ../../rav/*');
        console.log('Requesting Pairing Code...');
        await new Promise(resolve => setTimeout(resolve, 5000));
        
        try {
          let pairingCodeResult = await sock.requestPairingCode(phoneNumber, 'offer');
