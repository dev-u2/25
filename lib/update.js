    ...
    try {
        ...
        if (global.db?.groups[id] && store?.groupMetadata[id]) {
            ...
            for (let n of participants) {
                ...
                let messageText = "";
                if (action === "add") {
                    messageText = db.groups[id]?.setwelcome || Hi @user, welcome to @subject;
                    metadata.participants.push({ id: jidNormalizedUser(n), admin: null });
                } else if (action === "remove") {
                    messageText = db.groups[id]?.setleave || @user left the group @subject;
                    metadata.participants = metadata.participants.filter(p => !participants.includes(jidNormalizedUser(p.id)));
                } else if (action === "promote") {
                    messageText = db.groups[id]?.setpromote || @user was promoted to admin by @admin in @subject;
                    updateAdminStatus(participants, metadata.participants, "admin");
                } else if (action === "demote") {
                    messageText = db.groups[id]?.setdemote || @user was demoted from admin by @admin in @subject;
                ...
                if (messageText && rav.public) {
                    messageText = messageText
                        ...
                        .replace(/@desc/g, db.groups[id]?.desc || "No description");
